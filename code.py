...
def detect_pattern(df: pd.DataFrame) -> tuple[str, int | None]:
    ...
    Returns (name, index_of_detection) or ("None", None).
    """
    # ← your real detection goes here
    return "None", None
...
def get_24h_status() -> str:
    ...
    # Sun 8pm → Fri 8pm
    if wd < 4: return "Open"
    if wd == 4: return "Open" if t < time(20,0) else "Closed"
    if wd == 6: return "Open" if t >= time(20,0) else "Closed"
    return "Closed"
...
if not st.session_state.started:
    ...
    if st.button("▶️ Start Chart"):
        st.session_state.ticker  = t_input.strip().upper()
...
# ----- CHART SCREEN ----------------------------------------------------------
    ...
    with c1:
        ...
        st.markdown(
            f"<div style='background:{col};color:#fff;padding:20px;"
            f"text-align:center;font-size:24px;font-weight:bold;"
            f"border-radius:4px'>{sig}</div>",
        ...
        m24 = get_24h_status()
        st.markdown(
            "<div style='background:#112;color:#dde;padding:10px;"
            "border-radius:4px'>"
    ...
    with c2:
        ...
        st.markdown(
            "<div style='background:#023;color:#eef;padding:12px;"
            "border-radius:4px'>"
        ...
        st.markdown(
            "<div style='background:#432;color:#ffd;padding:12px;"
            "border-radius:4px'>"
